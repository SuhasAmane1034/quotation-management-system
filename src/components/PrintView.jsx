...
export default function PrintView({ quotation, onClose, settings }) {
  ...
  const accent = settings?.accent_color || '#1a1830';
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg)' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: 24, background: '#b8b7cf' }}>
        <div ref={printRef} style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12, paddingBottom: 12, borderBottom: `2.5px solid ${accent}` }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
              <div style={{ width: 58, height: 58, background: `linear-gradient(135deg,${accent},${accent}99)`, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 900, fontSize: 22, fontFamily: 'Syne,sans-serif', flexShrink: 0 }}>
            ...
            <div style={{ textAlign: 'right', minWidth: 160 }}>
              <div style={{ fontSize: 13, fontWeight: 800, letterSpacing: '0.18em', color: accent, fontFamily: 'Syne,sans-serif', marginBottom: 7 }}>QUOTATION</div>
                ...
                <div>
                  ...
                  <ol style={{ margin: 0, paddingLeft: 14 }}>
                    {parseTerms(quotation.terms).map((line, i) => {
                      ...
                      const isHigh = (clean === clean.toUpperCase() && clean.length > 4 && /[A-Z]{2}/.test(clean));
                      return (
                        <li key={i} style={{ fontSize: 10, marginBottom: 2, lineHeight: 1.5 }}>
                          <span style={{ color: isHigh ? '#cc0000' : '#444', fontWeight: isHigh ? 800 : 400 }}>{clean}</span>
                        </li>
                      );
                    })}
                  </ol>
