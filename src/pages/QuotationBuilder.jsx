...
function ProductCell({ value, onChange, onSelect, onKeyDown, userId }) {
          ...
          {results.map((p, i) => (
            <div key={p.id} className={`autocomplete-item ${i === idx ? 'selected' : ''}`} onMouseDown={() => pick(p)}>
              ...
              <div>
                ...
                <div className="autocomplete-item-meta">{p.code || '—'} · Rs.{p.rate}/{p.unit}{p.track_stock ? ` · Stock: ${p.stock}` : ''}</div>
              </div>
            </div>
          ))}
        </div>
...
export default function QuotationBuilder() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!q) { addToast('Quotation not found', 'error'); return; }
      const { data: items } = await supabase.from('quotation_items').select('*').eq('quotation_id', id).order('sr_no');
      setQuoteNumber(q.quote_number);
      setHeader({
        company_name: q.company_name, company_logo: q.company_logo || '',
      ...
      setRows(items?.length
        ? [...items.map(item => ({ ...item, _id: item.id || Math.random().toString(36).slice(2) })), emptyRow()]
        : [emptyRow()]);
      loaded.current = true;
    })();
  }, [id, user]);
      ...
      if (id) {
        ...
        if (validRows.length) {
          await supabase.from('quotation_items').insert(
            validRows.map((r, i) => ({
              quotation_id: id, user_id: user.id, sr_no: i + 1,
              product_id: r.product_id || null, product_name: r.product_name,
              product_image: r.product_image || '', shape: r.shape || '',
              color: r.color || '', body_color: r.body_color || '',
              warranty: r.warranty || '', quantity: r.quantity, unit: r.unit,
              rate: r.rate, discount: r.discount || 0, amount: r.amount,
              bill_after_warranty: r.bill_after_warranty || false,
        ...
        const quote_number = `QT-${String((count || 0) + 1).padStart(4, '0')}`;
        const { data: newQ, error } = await supabase.from('quotations').insert({
          ...header, user_id: user.id, quote_number
        }).select().maybeSingle();
        if (error) throw error;
        if (validRows.length) {
          await supabase.from('quotation_items').insert(
            validRows.map((r, i) => ({
              quotation_id: newQ.id, user_id: user.id, sr_no: i + 1,
              product_id: r.product_id || null, product_name: r.product_name,
              product_image: r.product_image || '', shape: r.shape || '',
              color: r.color || '', body_color: r.body_color || '',
              warranty: r.warranty || '', quantity: r.quantity, unit: r.unit,
        ...
        <div className="card">
          <div className="card-header">
            ...
            <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>
              Enter = next cell · Type shortcut in Shape/Color/Body/Warranty
            </span>
          </div>
                ...
                {rows.map((row, i) => (
                  <tr key={row._id}>
                    ...
                    <td data-row={i} data-field="product_name">
                      <ProductCell value={row.product_name} userId={user?.id}
            ...
            <div className="card-body">
              ...
              <div className="form-group" style={{ marginBottom: 0 }}>
                ...
                <textarea value={header.terms}
                  onChange={e => setHeader(h => ({ ...h, terms: e.target.value }))} rows={4} />
              </div>
            </div>
          </div>
